...
function initAllBars() {
  ...
  if (scoreFill) {
    ...
    animateBar(scoreFill, (target / 10) * 100, 300);
  }
...
function renderDayCard(data) {
  ...
  if (!data.exists) {
    ...
    content.innerHTML = `
      <div class="day-card-date">${data.date_display}</div>
      <div class="day-card-empty">
        <p>No data logged for this day.</p>
        
          href="/report/add/${data.date}/"
          class="protocol-submit-btn"
  ...
  content.innerHTML = `
    ...
    <div class="day-card-score-row">
      ...
      ">${data.score}</span>
      <div>
        <div style="font-size:24px;">${data.rank_icon}</div>
        <div class="day-card-rank">${data.rank}</div>
      </div>
    </div>

    <p class="day-card-ai-comment">${data.ai_comment}</p>
      ...
      <div class="stat-row">
        ...
        <div class="stat-info">
          <div class="stat-header">
            ...
            <span class="stat-value">${data.it_hours}h <span class="stat-target">/ 5h</span></span>
          </div>
  ...
  // Animate the day card's mini bars
  const dcBars = [
